...
* Notes: The AdGuard Base filter is supposed to be used with [Easylist](https://github.com/easylist/easylist) (like `AdGuard French` and `AdGuard Chinese`), so there's no need to add rules which are already in `Easylist`.